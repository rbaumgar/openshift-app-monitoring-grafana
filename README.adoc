...
* SSO remove resource.limits, replicas = 1
* OpenShift 4.13: podmonitor requieres a labal
(component=prometheusoperator msg="skipping podmonitor" error="relabel configuration for replace action needs targetLabel value" podmonitor=team-a/kafka-resources-metrics namespace=openshift-user-workload-monitoring prometheus=user-workload)
